...
/**
 * TypeSpecimen — a brand-kit typography cell. A large "Aa" glyph set in the
 * system family (Montserrat), the family label, and a live weights row where
 * each sample is rendered at the weight it names. CSS-only hover: the glyph
 * tightens its tracking on hover (motion-safe), no JS.
 */
  ...
  return (
    <div className={cn("flex h-full flex-col justify-between gap-4", className)}>
      <div className="flex items-start justify-between gap-4">
        <span
          className={cn(
            "select-none font-bold leading-[0.85] tracking-tight text-foreground",
      ...
      <div>
        <p className="text-sm font-semibold text-foreground">Montserrat</p>
        <p className="mt-0.5 text-xs text-muted-foreground">
          One family, worked across weights.
        </p>
